...
功能不是越多越好。默认低学习成本、低培训依赖；普通教师可见面只讲任务语言，不讲 `draft/test`、`candidate`、`active`、`medium_hard`、数值难度区间、脚本、证据、权限或审计术语。
...
2. 再执行 I001-I010：普通教师四入口、导入向导、人工确认队列、找题组卷、成绩导入分析、新手默认值、前端边界、简洁模式、教师可见术语语义漏检和教师 shell 后台边界收口。
3. 再执行 J001-J006：真实 docx/PDF/扫描件 Adapter、公式/表格/题图保真、工具诊断、导入准确率与人工工作量报告。
4. K 以后必须等前置证据通过再进入，不因长期路线图已写出就提前扩大功能面。进入 L0/M0/N0/O0/P0-live 前先完成并保持 I010；进入 P0-live 前还必须完成 O004B 角色权限与审计日志剩余闭环。
