...
长期默认路线与禁止提前做事项，见 `docs/decisions/ADR-014-recommended-engineering-endstate-and-stack-boundary.md` 与 `docs/110_EngineeringEndStateChecklist.md`。
...
2026-06-07 当前仓内用 `configs/ai-provider-profiles.defaults.yaml` + `AiRoutingControlPanel` contract 固化 `NS1305` 的角色路由和 provider profile 语义：普通教师只看到离线优先、云 API 增强、本地增强三种简化模式；管理员才可查看 provider profile 的 env 引用、base URL、并发、预算、fallback 和禁用开关。业务代码继续按 role / tier 路由，不在 `AiModelRouter` 或 `appsettings.json` 中写死具体模型名；所有 provider secret 只允许用 env 引用，任何真实 key、云调用启用或生产默认切换都必须走 no-active-write、budget/cache、schema/eval 和人工确认门禁。
