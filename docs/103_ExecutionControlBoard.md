...
AI 推荐：当前唯一主目标不是继续扩能力面，而是把 `NS13 -> P001 -> P005/P006` 收成一个可裁决、可发布、可回滚的单主线。
...
| 主线 | 目标 | 退出条件 | 负责人角色 |
|---|---|---|---|
| `NS13` 产品化运行形态 | 收束 Windows Service、控制面板、profile、role-routed AI、自动化边界、golden/eval/security | `NS1301-NS1308` 达到可宣称状态，且不再把开发机偶然环境当产品默认 | 工程负责人 + 发布负责人 |
...
| `NS1301-NS1308` | 待办 | 运行形态与发布前置收口，不再依赖个人开发机默认环境 |
