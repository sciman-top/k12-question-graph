...
2026-06-09 已将本地参考库同步到最新 HEAD，并按当前项目主线做了一轮增删：补入 `PowerShell-Docs`、`playwright`、`react-router`、`paperless-ngx`，移除本地 `openedx-platform` 镜像；各仓当前验证提交号以 `references.manifest.json` 为准。仓内快照则通过 `tools/sync-reference-shelf-snapshot.ps1` 从外部 manifest 同步，避免 CI 因 snapshot 漂移误判。
...
Moodle、OpenOLAT、TAO 等成熟教育平台可参考权限、题库、测试和标准边界；Paperless-ngx 可参考本地文档归档、OCR、搜索、备份和“本地服务 + 数据目录”思路；题库生成类 GitHub 项目可参考“按知识点、题型、难度选题”的基本思路。但这些项目通常不以中国学校教师的 Word/Excel 流程效率为最高约束，也往往缺少 Word/PDF 入库、公式题图处理、成绩分析、低成本 AI 流水线和离线备份恢复闭环。
