...
- .NET 10 LTS 与 EF Core 10：适合作为 v0.1 的默认主线，但必须锁定 SDK/runtime 检查和 migration gate。
- ASP.NET Core BackgroundService / Windows Service：支持先用单体后台服务跑 P0/P1 job，后续再按证据引入 Hangfire/RabbitMQ。
- PostgreSQL JSONB/Full Text Search/pg_trgm/pgvector：用一个数据库覆盖半结构化字段、全文、模糊和向量检索。
