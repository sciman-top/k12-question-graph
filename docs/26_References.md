...
使用边界：本地浅克隆用于 `rg` 检索、结构阅读和技术决策复核；官方语义仍以官网/当前版本文档为准。默认只更新 `core` 参考集；`dotnet-eShop`、`openedx-platform`、`OpenOLAT`、`moodle` 和 `pgvector` 作为 `optional`，只在明确研究对应方向时更新。仓库分组、上游 URL、用途说明、最近一次验证提交号和补充说明都以外部 `references.manifest.json` 为单一真相入口。Moodle、Open edX、OpenOLAT 等大型教育平台只提炼题库、测评、课程资产、权限、审计和迁移做法，不复制其完整 LMS 路线。
