...
2. 把“Web 常驻 + API 按需拉起”的当前运行模型整理成一份更清晰的运维说明。
3. 给教师主链路补一份更紧凑的浏览器可见证据总览。
4. 对 full gate 中最重的几段演练继续做减重，而不是重复功能性修复。
5. 若要追求真正“项目终态完成”，下一跳必须转入 `P001 / REAL005` 的现场级证据，而不是继续在本机夸大关闭状态。
