...
- 日期：2026-07-28
- 状态：设计文档已确认并实施中；截至 2026-07-29，2015-2025 真题双层入库门禁、考查目标候选和年报候选持久化已完成 repo-side 验证
- 设计选择：方案 3，多层证据模型
...
2026-07-29 的数据库与来源证据基线为：2015-2020 每年 24 题、2021-2025 每年 18 题，共 234 题；234 条题干、答案内容、试卷锚点、答案锚点和题级年报锚点齐全，`questionCorpusReady/reportEvidenceReady/allFieldExtractionReady=true`。年报候选锚点共物化 268 个 SourceRegion，其中 2015 的 24 个题级区域由“题号标题 + 题干相似度”定位，仍须教师回看确认。该基线只证明候选提炼前置条件成立，不代表审核、生产激活或 REAL005 闭环。
...
开始任何知识点、考查目标或年报字段提炼前，必须先运行数据库就绪门禁。门禁分两层：

- `questionCorpusReady`：2015-2025 逐年题号序列、234 道 `QuestionItem`、非空题干、非空答案内容、试卷/答案 `SourceDocument + SourceRegion` 锚点全部存在，且全部保持候选状态；只有该层通过，才能生成知识点和 `AssessmentTarget` 候选。
- `reportEvidenceReady`：每道题另有可回看的年报题级锚点；只有该层通过，才能把年报实测指标、典型错误和教学建议标为完整提炼。只有文档级年报入库或 PDF 页码候选，不等于该层通过。

缺年、缺题、重复题号、空题干、空答案、缺来源锚点或 production eligible 漂移均 fail-closed；不得用文件存在、整份文档回退或旧 CSV 行数替代数据库逐题证明。
