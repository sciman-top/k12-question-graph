...
| 风险 | 影响 | 可接受条件 | owner |
|---|---|---|---|
| `NS13` 未完成 | 发布形态、安装、profile、AI 路由和 release evidence 仍不是产品默认形态 | 完成 `NS1301-NS1308` 并补齐证据 | 工程负责人 + 发布负责人 |
...
1. `NS1301-NS1308` 完成并回写机器清单与证据。
2. `P001` 只剩隔离机安装、打印、网络、权限域和操作者签收等现场事实，并已完成对应签收。
3. `P003` 数据授权、支持负责人、回滚方案和反馈模板完成。
4. `P005` 反馈完成 keep / modify / defer / do_not_do 分流。
5. `P006` 形成正式 release decision record，并明确 tag candidate 与 rollback window。
