...
- D001 AI Provider 抽象与 ModelRouter。已完成 draft/test 合同：`AllowRealModelCalls=false`，LLM 任务只路由到 `stub_llm`，保留 prompt/schema/model routing 版本、成本档位、人工审核和 production guard，不把 draft 知识点或真实模型输出标记为生产完成。
- D002 Model Router。
...
若 C002 仍为暂缓，P3 不再被完全阻断，但只能进入 draft/test 模式：允许实现 provider 抽象、schema、prompt/schema/model routing 版本记录、Evals、成本日志、人工审核和迁移建议；不得把 draft bootstrap 知识点作为生产正式输入，不得把真实模型输出自动写入正式 `active` 知识体系。D001 当前只证明路由和禁用真实模型调用的合同成立，不代表可进行生产 AI 标注。
