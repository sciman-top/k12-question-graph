...
- C002B draft -> formal 替换映射：支持等价、拆分、合并、上位、下位、废弃、重命名；规则和 AI 先自动生成建议，少量低置信度或高影响项由人工审核。
...
若 C002 仍为暂缓，P3 不再被完全阻断，但只能进入 draft/test 模式：允许实现 provider 抽象、schema、prompt/schema/model routing 版本记录、Evals、成本日志、人工审核和迁移建议；不得把 draft bootstrap 知识点作为生产正式输入，不得把真实模型输出自动写入正式 `active` 知识体系。
