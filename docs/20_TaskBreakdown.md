...
- C002K C002 cleaned candidate 自动导入 candidate DB。已完成：以旧 `c002-k12-question-graph-candidate-csvs\cleaned` 为输入，写入 92 个候选动态资产、55 条 `pending_review` 映射、1 个 `pending_review` migration 计划和 1 个 `c002_candidate_import` 审核队列项。`guangzhou-physics-full-research-package-2016-2025\csv` 后续提供了更完整的 C003 研究包数据，现可通过 `tools/prepare-c002-candidate-csvs.ps1 -InputDir 'guangzhou-physics-full-research-package-2016-2025\csv' -OutputDir 'c002-k12-question-graph-candidate-csvs\cleaned'` 转换成既有 C002 candidate import 输入，用于替换旧残缺候选数据；真实 DB 替换仍必须先完成 dry-run、source hash 对齐、备份 manifest、人工审核边界和 active guard。所有资产保持 `candidate`，所有映射保持 `pending_review/auto_applied=false`，`active` 数量为 0；正式 C002 仍需人工审核、影响确认和激活 guard。
...
- C002S 广州物理正式化前审查闭环。已建立阻断 guard，仍暂缓 active：`tools/run-c002s-formalization-precheck.ps1` 已抽样核对 2016-2025 每年 3 道题的原卷、答案、年报页码、考点、知识点、教材/课标映射，报告为 `docs/evidence/c002s-formalization-precheck-report.json`；当前抽样核对通过，但 210 条年报页码/指标质量问题仍未清零，`productionActivationAllowed=false`。后续必须清零质量问题，再跑 candidate DB dry-run、备份 manifest、审核队列 readiness 和 active guard。未通过前只能保持 `candidate/pending_review/production_eligible=false`。
...
- G001 自动备份到本机与共享目录。暂缓：draft/test 模式下备份包通过 manifest/hash 校验，主程序不可用时脚本仍可运行，共享目录路径作为动态配置可修改。
- G002 缓存清理与存储看板。暂缓：draft/test 模式下管理员能看到存储占用并执行安全清理，保留配置化目录和回滚证据。
- G003 WinPE 应急拷贝脚本生成。暂缓：draft/test 模式下可生成离线恢复说明和应急拷贝脚本，路径和介质策略作为动态配置可修改。
- G004 安装器数据库凭据初始化与 pgpass 验证。暂缓：安装器或初始化向导在新电脑上写入或更新 PostgreSQL 非交互凭据存储，优先 `%APPDATA%\postgresql\pgpass.conf` 或等价安全凭据机制；验收时必须清除进程级 `PGPASSWORD` 后用 `psql -w` 验证连接，检查 ACL、回滚和修复报告，不依赖 Codex 或桌面进程自然继承 User 环境变量。
