...
- C002K C002 cleaned candidate 自动导入 candidate DB。已完成：质量复核完成包通过 `tools/merge-c003-quality-review-package.ps1` 叠加到完整 C003 CSV 包后，`tools/prepare-c002-candidate-csvs.ps1` 已生成新的 cleaned candidate 输入；`tools/import-c002-candidate-assets.ps1` 经 dry-run、source hash 对齐和备份 manifest `D:\KQG_Backups\20260504-013307\manifest.json` 后执行 candidate apply。当前 DB 中该批次保持 452 个候选动态资产、400 条 `pending_review` 映射、1 个 `pending_review` migration 计划和 1 个 `c002_candidate_import` 审核队列项，`active` 数量仍为 0；正式 C002 仍需人工审核、影响确认和激活 guard。
