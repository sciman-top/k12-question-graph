...
| replacement_mapping | 等价、拆分、合并、上位/下位、交叉覆盖、废弃、重命名 |
| mapping_cardinality | one_to_one/one_to_many/many_to_one/many_to_many |
...
规则和 AI 可以先做自动匹配、替换和迁移建议，目标是减少教师维护工作。自动化只能处理高置信度、低影响、可回滚的一对一变更；影响历史成绩、正式组卷、校级统计或一对多、多对一、多对多的变更必须保留人工审核。

多对多不要求一张表行同时保存多个 source 和多个 target。基础表示可以是多条 `DomainAssetMapping` 边，并用同一个 migration/plan 分组表达一个迁移集合。系统必须在影响报告中展示这个集合的业务含义，例如“旧能力维度 A/B 同时拆解并合并到新能力维度 C/D/E”，而不是把多条边当成互不相关的一对一替换。

## 10. 必须动态化的对象清单

以下对象不得在代码里写死为不可迁移常量；若短期为了 UI 或测试使用静态候选，也必须有后续迁移到 `DomainAssetVersion` 或等价版本化表的归宿：

| 类别 | 对象 | 常见变化原因 | 最低要求 |
|---|---|---|---|
| 学科知识 | 知识点、知识边、能力维度、易错点、实验方法 | 课标、教材、地区考试、校本教研变化 | 版本、来源、替换映射、题目绑定迁移 |
| 教材课标 | 教材版本、章节目录、课程标准条目、地区考点 | 教材改版、政策调整、地区命题变化 | 作为映射层，不覆盖知识本体历史 |
| 题目属性 | 题型、标签、难度、分值规则、rubric、评分细则 | 学科差异、考试口径、教师习惯变化 | 可停用、合并、拆分、别名和审核 |
| 导入解析 | 文档解析 pipeline、OCR/adapter 版本、切题策略、错误类型 | 工具升级、样本变化、学校资料格式变化 | 版本化配置、diagnostics、可重跑 |
| AI 策略 | prompt、schema、model routing、成本策略、eval 集 | 模型升级、成本变化、schema 调整 | 版本、输出可解释、旧结果可复算或冻结 |
| 组卷导出 | 组卷规则、细目表模板、导出模板、版式偏好 | 教师偏好、学校模板、考试要求变化 | 模板版本、默认值继承、回滚 |
| 成绩学情 | Excel 字段映射、小题分结构、分析指标、分层规则 | 班级模板、考试结构、教研口径变化 | 映射模板、指标版本、历史报告冻结 |
| 组织权限 | 学校、年级、班级、教研组、角色、共享策略 | 学校组织调整、权限政策变化 | 生效范围、历史 owner、审计和回滚 |
| 安全合规 | 隐私策略、保留策略、外部 AI 传输边界 | 法规、校规、部署环境变化 | policy 版本、证据、阻断和审计 |
| 互操作 | QTI/CASE/OneRoster/Caliper 映射 | 标准升级、第三方系统差异 | 适配层版本，不污染内部主模型 |
