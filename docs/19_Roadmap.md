...
2026-06-20 状态刷新补充：最新完整 `full gate` 仍是 2026-06-09 通过；2026-06-20 又刷新了 `live-pilot closeout plan guard`、`live-pilot closeout repo-side audit`、`REAL005` closure standard 以及相关 PQR 守卫。`REAL005B` 与 `REAL005C` 的 repo-side evidence 已完成，但 `REAL005` 仍保持 `not_closed`、`fullClosureAllowed = false`，当前 next open 已推进到 `REAL005D`，`P001/P003/P005/P006 = 待办`。因此路线图的最新真实边界仍不是“继续扩大功能面”，而是继续把高风险改动放在 `reference-basis` 与 truthful closeout 约束下推进。
...
截至 2026-05-16，`REAL006/REAL007` 已从症状修复推进到可复跑门禁：`REAL004` smoke 会强制检查 2015 第 1-24 题来源裁图、整页图和必需题图资产 URL；`tools/run-real007-guangzhou-2015-layout-quality.ps1` 会重建截图、写入 `source_region_revision_batch` audit，并输出缺图、JSON 占位截图、版面噪声重叠和题图资产覆盖报告。当前报告覆盖 67 个来源区域、24 题，缺图 0、占位 0、噪声重叠 0、必需题图缺失 0。2026-05-18 补完 `REAL008`：`POST /questions/{id}/assets`、`DELETE /questions/{id}/assets/{assetId}` 和 `tools/run-real008-question-asset-smoke.ps1` 证明题库卡片 `hasImage/assetCount`、题目详情题图和来源回看均来自真实 `QuestionAsset`，且关联、解除关联、重新关联都有 audit。同日补完 `REAL009`：`tools/run-real009-table-structure-smoke.ps1` 证明表格以 `QuestionBlock table` 结构化保存，保留来源截图，低置信度进入 `question_table_block_review` 审核队列，且不会被误判为题图资产。`REAL010` 也已补完：OpenXML worker 保留 OMML 第一真源，`tools/run-real010-formula-fidelity-smoke.ps1` 证明 Office 公式保存 OMML/LaTeX/MathML，扫描公式保留 fallback 图并进入 `question_formula_block_review`。`REAL011` 已补完 API 级异常编辑：`PATCH /questions/{id}` 可修订题干、答案、解析、题型、分值、难度、active 知识点和 block，并会保留 `questionNo/exam` 元数据、写入 primary `knowledge_mappings`；`PATCH /source-regions/{id}` 可重裁来源框，两者都有 audit。`REAL012` 已补完真实生产使用抽样链：`tools/run-real012-production-flow-quality-smoke.ps1` 证明 2015 广州真题样题进入题号排序检索、题篮、导出预检、Word/PDF 草稿产物和学情讲评引用，并新增 `GET /source-documents/{id}/quality-report` 输出题号、答案、题图、表格/公式、待人工项、噪声、AI 调用和 rollback SQL；当前逐卷报告仍因历史残留无题号题块、待人工项和缺截图项保持 `not_closed`，所以 `REAL005` 仍不得关闭。
...
2015-2025 真卷全流程闭环不得用局部 smoke 代替。`tools/run-real005-guangzhou-2015-2025-closure-standard.ps1` 是当前判定入口：它读取 `tasks/real-guangzhou-closure-criteria.csv`、`tasks/backlog.csv` 和 `tasks/completion-state-dashboard.csv`，输出最新日期化 evidence，例如 `docs/evidence/20260620-real005-guangzhou-2015-2025-closure-standard-report.json`。截至 2026-06-20，`REAL005A/B/C` 已按 repo-side evidence 完成，但报告仍保持 `closureStatus=not_closed`、`fullClosureAllowed=false`；当前 next open 已推进到 `REAL005D`，它只负责继续把 README / 当前闭环状态 / 发布卡等对外口径保持为 truthful `not_closed`。只要逐年/逐题证据缺来源 hash、答案覆盖、截图级 SourceRegion、题图资产、教师 audit、检索组卷导出和学情引用，或者现场 / 隔离机 / 签收边界未关闭，报告就必须继续保持 `closureStatus=not_closed`。
