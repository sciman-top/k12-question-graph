...
- 当前 G001-G003 已完成 draft/test 合同：备份到本机/共享目录、管理员存储看板/缓存清理、WinPE 应急拷贝脚本生成均已纳入 unified gate；缓存清理只作用于配置化 cache root，WinPE 脚本只做 copy-only 拷贝，不删除文件仓库、备份包、学生成绩、正式资产或目标介质既有内容。
