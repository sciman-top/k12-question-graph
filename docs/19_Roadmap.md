...
真实导入是中风险持久化动作，执行顺序固定为：确认 `git status`、设置正确 `PGPASSWORD/KQG_CONNECTION_STRING`、先运行 dry-run、执行并校验可恢复 manifest、再运行 `tools/import-c002-source-materials.ps1 -Apply -BackupManifest '<manifest.json>'`。若数据库密码或备份 manifest 缺失、不匹配，导入必须停在 dry-run 和任务更新层，不得绕过来源证据链直接导入候选知识点。
...
2015-2025 真卷全流程闭环不得用局部 smoke 代替。`tools/run-real005-guangzhou-2015-2025-closure-standard.ps1` 是当前判定入口：它读取 `tasks/real-guangzhou-closure-criteria.csv`、`tasks/backlog.csv` 和 `tasks/completion-state-dashboard.csv`，输出最新日期化 evidence，例如 `docs/evidence/20260623-real005-guangzhou-2015-2025-closure-standard-report.json`。截至 2026-06-23，`REAL005A/B/C/D 的 repo-side closeout 已完成`，但报告仍保持 `closureStatus=not_closed`、`fullClosureAllowed=false`；`REAL005` 的 repo-side next open 已收口为 `none`。只要 completion dashboard 仍保持 `contract_done`、现场 / 隔离机 / 签收边界未关闭，报告就必须继续保持 `closureStatus=not_closed`。
