...
C002 active 前必须先完成 C002S 正式化前审查闭环：抽样核对 2016-2025 每年 2-3 道题的原卷、答案、年报页码、考点、知识点、教材/课标映射；把 C003 研究包中 210 条年报页码/指标质量问题清零；再通过 candidate DB dry-run、备份 manifest、审核队列 readiness 和 active guard。当前 `tools/run-c002s-formalization-precheck.ps1` 已把抽样核对和质量问题阻断脚本化，报告为 `docs/evidence/c002s-formalization-precheck-report.json`；抽样核对通过，但 210 条质量问题仍使 `productionActivationAllowed=false`。未通过前，`guangzhou-physics-full-research-package-2016-2025` 只能是高质量候选包，不能升为 `active`。
