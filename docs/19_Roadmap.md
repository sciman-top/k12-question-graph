...
截至 2026-05-04，`D:\CODE\k12-question-graph\广州中考` 已完成 C002J 真实来源资料导入：课程标准 1 份、教材 3 份、广州中考年报 10 份、广州中考真题/答案/解析 19 份，共 33 个 PDF，已进入 `SourceDocument/FileAsset` 证据层，真实 `material_batch_key` 为 `guangzhou_physics_2016_2025`。C002K 已从质量复核完成后的 C003 合并包导入候选 DB：452 个 `candidate` 动态资产、400 条 `pending_review` 映射、1 个 `pending_review` migration 计划和 1 个审核队列项；source hash 对齐为 33/33，`active` 数量为 0，备份 manifest 为 `D:\KQG_Backups\20260504-013307\manifest.json`。正式 C002 仍不得标记为完成或 active，必须等人工审核、影响确认、回滚快照和 active guard 全部通过。
