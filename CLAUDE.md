...
- 不在本文件复制项目事实、门禁、证据、回滚或 `Global Rule -> Repo Action`；若共同规则要变，直接修改本仓根 `AGENTS.md` 并复测。
...
- 若本仓存在 `.claude/settings.json` 或 `.claude/hooks/`，按本仓事实维护；不要假定存在外部治理下发。
...
## D. 维护校验
- 本 wrapper 不改写 `AGENTS.md` 的 A/C/D 项目事实；如发现共同规则与 Claude 差异冲突，先按代码、gate 和 `AGENTS.md` 事实定位，再直接修本仓规则。
