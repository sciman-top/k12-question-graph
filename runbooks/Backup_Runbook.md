...
- 策略 b 的 apply restore 要求目标 `{DataRoot}\config` 不存在或为空；`-AllowOverlay` 只适用于 FileStore/config recovery 目标，不会豁免该运行时配置闸门。若目标仍有旧 `ai-provider-settings.local.json` 或 key ring，先按现场快照/回滚流程隔离或清除后再重试。
