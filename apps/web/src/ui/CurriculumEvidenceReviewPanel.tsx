...
function collectSourceAnchors(value: unknown, inheritedRole = '', result: SourceAnchor[] = []) {
  ...
  const duplicateIndex = result.findIndex((entry) => sourceRegionId
    ? entry.sourceRegionId === sourceRegionId
    : entry.sourceDocumentId === sourceDocumentId && entry.pageNumber === pageNumber)
  if (duplicateIndex >= 0 && !result[duplicateIndex].role && role) {
    result[duplicateIndex] = { ...result[duplicateIndex], role }
  } else if (duplicateIndex < 0 && (sourceRegionId || (sourceDocumentId && pageNumber))) {
    result.push({
      role,
      sourceRegionId,
      sourceDocumentId,
