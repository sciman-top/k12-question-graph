...
const teacherSimpleModes = [
  ...
  },
  {
    ...
    summary: '管理员单独启用 Cockpit 本地 API 网关，模型按 Astra、Sol、Terra、Luna、GLM Flash、DeepSeek V4.1 Flash 受控切换。',
...
const providerProfiles = [
  ...
  },
  {
    ...
    concurrency: '2',
    budget: '0 元 / 月',
    fallback: 'Astra -> Sol -> Terra -> Luna -> GLM Flash -> DeepSeek V4.1 Flash -> pending_review',
    status: '默认关闭',
...
const modelPresets = [
  {
    id: 'astra',
    model: 'GPT-6-Astra-only · gpt-6-astra',
    reasoningEfforts: 'quality=astra·high / balanced=astra·medium / economy=astra·low',
    fallback: '首选；故障后 Sol -> Terra -> Luna -> GLM Flash -> DeepSeek V4.1 Flash',
  },
  {
    id: 'sol',
    model: 'GPT-5.6-Sol-only · gpt-5.6-sol',
    reasoningEfforts: 'quality=sol·high / balanced=sol·medium / economy=sol·low',
    fallback: '次选；故障后 Astra -> Terra -> Luna -> GLM Flash -> DeepSeek V4.1 Flash',
  },
  {
    id: 'terra',
    model: 'GPT-5.6-Terra-only · gpt-5.6-terra',
    reasoningEfforts: 'quality=terra·max / balanced=terra·xhigh / economy=terra·high',
    fallback: '第三顺位；故障后 Astra -> Sol -> Luna -> GLM Flash -> DeepSeek V4.1 Flash',
  },
  {
    id: 'luna',
    model: 'GPT-5.6-Luna-only · gpt-5.6-luna',
    reasoningEfforts: 'quality=luna·max / balanced=luna·xhigh / economy=luna·high',
    fallback: '第四顺位；故障后 Astra -> Sol -> Terra -> GLM Flash -> DeepSeek V4.1 Flash',
  },
  {
    id: 'glm_flash',
    model: 'GLM Flash · glm-5.3-flash',
    reasoningEfforts: 'quality=GLM·max / balanced=GLM·high / economy=GLM·low',
    fallback: '默认链第五顺位；故障后 DeepSeek V4.1 Flash',
  },
  {
    id: 'deepseek_flash',
    ...
    >
      <div className="panel-heading">
        <div>
          ...
          <Typography.Text type="secondary">
            普通教师只看离线优先与 Cockpit 本地 API 增强等简化模式；管理员只能使用固定本地网关，模型故障按 Astra/Sol/Terra/Luna/GLM/DeepSeek preset 顺序切换。
          </Typography.Text>
        </div>
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item label="默认试跑 preset" name="defaultSmokeModel" rules={[{ required: true }]} extra="默认以 Astra 的 gpt-6-astra 为首选；实际故障切换由当前完整 preset 决定。">
            <Input readOnly />
          </Form.Item>
