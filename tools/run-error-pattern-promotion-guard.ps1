...
$errorPatternScopeStart = $serviceSource.IndexOf(
    'internal static IReadOnlyList<ErrorPatternCandidate> BuildErrorPatternCandidates(',
    [StringComparison]::Ordinal)
$errorPatternScopeEnd = $serviceSource.IndexOf(
    'private static Guid CreateDeterministicGuid(',
    [StringComparison]::Ordinal)
Assert-True ($errorPatternScopeStart -ge 0 -and $errorPatternScopeEnd -gt $errorPatternScopeStart) 'CEK-20 service method scope could not be isolated'
$errorPatternSource = $serviceSource.Substring($errorPatternScopeStart, $errorPatternScopeEnd - $errorPatternScopeStart)
...
foreach ($requiredSourceToken in @('BuildErrorPatternCandidates','DomainAssetStatuses.Candidate','DomainAssetReviewStatuses.PendingReview','RequiresC002RImpactReport: true','RequiresRollbackSnapshot: true','AutoApplyAllowed: false')) {
    Assert-True ($errorPatternSource.Contains($requiredSourceToken, [StringComparison]::Ordinal)) "CEK-20 service is missing contract token: $requiredSourceToken"
}
Assert-True (-not $errorPatternSource.Contains('SaveChanges', [StringComparison]::Ordinal)) 'CEK-20 candidate builder must remain read-only'
Assert-True (-not $errorPatternSource.Contains('dbContext.', [StringComparison]::Ordinal)) 'CEK-20 candidate builder must not access DbContext'
...
try {
    dotnet test tests\api\K12QuestionGraph.Api.Tests\K12QuestionGraph.Api.Tests.csproj `
        --filter ErrorPattern `
        --logger 'trx;LogFileName=error-pattern-promotion.trx' `
...
$report = [ordered]@{
    ...
    governance = [ordered]@{
        ...
        readOnlyScope = 'BuildErrorPatternCandidates through BuildPromotionDecision'
    }
