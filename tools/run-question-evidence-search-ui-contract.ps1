...
$workbenchHookPath = Join-Path $repoRoot 'apps\web\src\ui\useQuestionEvidenceWorkbench.ts'
...
$workbenchHook = Get-Content -LiteralPath $workbenchHookPath -Raw
$appBoundary = $app + "`n" + $workbenchHook
...
foreach ($marker in @(
    ...
    Assert-Condition ($appBoundary.Contains($marker)) "missing CEK-30 app/workbench hook marker: $marker"
}
...
$report = [ordered]@{
    ...
    evidence = [ordered]@{
        ...
        workbenchHook = 'apps/web/src/ui/useQuestionEvidenceWorkbench.ts'
