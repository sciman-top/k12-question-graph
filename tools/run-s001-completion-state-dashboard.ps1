...
Assert-True ($s002.status -eq '待办') 'S002 must remain next unfinished productization task'
Assert-True ($s012.status -eq '待办') 'S012 must remain pending until non-onsite E2E rehearsal is done'
...
$report = [ordered]@{
    ...
    nextProductizationTask = 'S002'
    conclusion = '当前项目拥有可验证底座和合同能力 但教师可直接连续使用的 release_ready 板块为 0 必须先执行 S002-S012'
}
...
$lines.Add("- next_productization_task: S002")
$lines.Add('')
