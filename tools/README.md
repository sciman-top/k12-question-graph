...
does not prove teacher acceptance or authenticated authorization. CEK-33
browser acceptance and CEK-34 full gate later passed, but CEK-20 persistence,
`REAL005`, and a separately authorized production switch remain open. The evidence report is
...
# Only after fresh explicit confirmation of the PostgreSQL/API process impact:
pwsh -NoProfile -ExecutionPolicy Bypass -File tools/run-curriculum-exam-knowledge-evidence-suite.ps1 `
  -FullGateAuthorized
```

The default command runs API and Web builds, the complete API/Worker/Web test
suites, Web lint, CEK schema/evidence contracts, roadmap/reference guards,
NuGet/npm vulnerability checks, and a manifest-driven restore into a unique
`kqg_cek034_*` database plus a unique `tmp/cek034-*` FileStore root. It
verifies all restored FileStore SHA-256 values, migrates CEK-15/19 Down and Up,
compares production boundary counters before and after, and deletes only the
generated database/root in `finally`.

Without `-FullGateAuthorized`, `tools/run-gates.ps1` is recorded as
`not_authorized`; the report exits successfully but remains
`status=partial/cek34Complete=false`. The explicit switch runs the full gate
in the fixed test/full phase. Evidence is written to
`docs/evidence/cek034-curriculum-exam-knowledge-evidence-suite.json`. Neither
mode approves candidates, switches production active assets, proves teacher or
identity authorization, closes `REAL005`, or changes release No-Go.

The fresh authorized run on 2026-07-31 completed with
`status=pass/cek34Complete=true/fullGateAuthorized=true`: API 78, Worker 182,
Web 38, lint, the repository full gate, 19 referenced CEK evidence files,
isolated restore/hash/migration checks, and supply-chain scans passed. This is
CEK repo-side closure only; production active remains unchanged and external
