...
$idempotencyChecks = @($import.idempotencyChecks)
$explicitIdempotencyPass = $idempotencyChecks.Count -eq 33 -and @(
    $idempotencyChecks | Where-Object {
        $_.pass -ne $true -or
        $_.sourceDocumentId -ne $_.repeatedSourceDocumentId -or
        $_.fileAssetId -ne $_.repeatedFileAssetId
    }
).Count -eq 0
$duplicateReplayPass = @($import.uploaded | Where-Object { $_.isDuplicate -ne $true }).Count -eq 0
$checks = [ordered]@{
    ...
    idempotentSourceIds = $sourceIds.Count -eq 33 -and ($explicitIdempotencyPass -or $duplicateReplayPass)
